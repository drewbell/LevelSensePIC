...
/*----------------------------- Module Defines ----------------------------*/
typedef boolean InitFunc_t(uint8_t Priority);
...
// TODO: static pPostFunc const pPostKeyFunc = POST_KEY_FUNC;
...
ES_Return_t ES_Initialize(TimerRate_t NewRate) {
    unsigned char i;
    ES_Sys_Timer_Init(); // start up the timer subsystem
...
ES_Return_t ES_Run(void) {
    ...
    while (1) { // stay here unless we detect an error condition
        ...
        while (Ready != 0) {
            ...
            //                return FailedRun;
            //            }
            // TODO: implementing as switch-case to allow for managed stack implementation
            switch (HighestPrior) {
                case 0:
                    if (SERV_0_RUN(ThisEvent).EventType != ES_NO_EVENT) return FailedRun;
                    break;
                case 1:
        ...
        while (IsTimerPending() > 0) // if timer has ticked, then post those messages
            ES_Timer_RTI_Resp(); // this is a work-around for the PIC
 ...
 ****************************************************************************/
boolean ES_PostToService(uint8_t WhichService, ES_Event TheEvent) {
